    ...
    let food = new Food(fields);
        ...
        food.save((err, food) => {
            if(err){
                console.log(err)
                res.status(400).json({
                    error: "Saving Dish to the Database failed"
                });
            }
...
exports.getMenu = (req, res) => {
    ...
    Food.find({rest:req.profile._id,dishStock: { $gt: 0 }})
    //.populate("merchant")
    .sort([[sortBy, "asc"]])
    .exec((err, foods) => {
        if(err){
...
exports.updateStockOnCancel = (req, res, next) => {
    let myOperations = req.order.foods.map(prod => {
        return{
            updateOne: {
                filter: {_id: prod.food},
                update: {$inc: {dishStock: +prod.count, sold: -prod.count}}
...
exports.updateStockOnAccept = (req, res, next) => {
    let myOperations = req.order.foods.map(prod => {
        return{
            updateOne: {
                filter: {_id: prod.food},
